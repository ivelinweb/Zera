    ...
    const fetchData = async () => {
      if (window.ethereum) {
        try {
          ...
          if (chainId !== CHAIN_CONFIG.BNBMainnet.chainId && chainId !== CHAIN_CONFIG.BNBTestnet.chainId) {
            toast({
              title: "Wrong Network",
              description: "Please switch to BNB Smart Chain Mainnet or Testnet",
              variant: "destructive",
            });
            return;
          }
          ...
          const contractAddress =
            (chainId === CHAIN_CONFIG.BNBMainnet.chainId
              ? CONTRACT_ADDRESSES.BNBMainnet
              : CONTRACT_ADDRESSES.BNBTestnet);
          ...
          try {
            ...
            for (let i = 0; i < total; i++) {
              try {
                ...
                const chainName = chainId === CHAIN_CONFIG.BNBMainnet.chainId ? "BNB Mainnet" : "BNB Testnet";
                allAudits.unshift({
                  id: i + 1,
                  name: `Contract ${i}`,
                  chain: chainName,
