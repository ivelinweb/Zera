if (!process.env.NEXT_PUBLIC_ZAI_API_KEY) {
  throw new Error(
    "Missing required environment variables: NEXT_PUBLIC_ZAI_API_KEY"
  );
}
...
export async function callZaiAI({
  messages,
  model = "glm-4-plus",
  temperature = 0.7,
  maxTokens = 2000,
  ...
  try {
    ...
    console.error("Error calling Z.ai (GLM 5.1):", error);
    throw error;
  }
}
